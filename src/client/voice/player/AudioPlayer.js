...
class AudioPlayer extends EventEmitter {
  ...
  playUnknownStream(stream, { seek = 0, volume = 1, passes = 1, tempo = 1, pitch = 44100 } = {}) {
    OpusEncoders.guaranteeOpusEngine();
    const options = { seek, volume, passes, tempo, pitch };
  ...
  playOpusStream(stream, { seek = 0, passes = 1, volume = 1 } = {}) {
    const options = { seek, passes, volume, opus: true };
    this.destroyAllStreams(stream);
    const dispatcher = this.createDispatcher(stream, options);
